...
function hourInRange(hour: number, startHHMM: string, endHHMM: string): boolean {
  const start = parseHHMMToMinutes(startHHMM);
  const end = parseHHMMToMinutes(endHHMM);
  const t = hour * 60;
  // If end == start, treat as "no range"
...
function shiftBorderForHour(
  hour: number,
  shifts: { morning: { start: string; end: string }; afternoon: { start: string; end: string }; guard: { start: string; end: string } }
): string | null {
  if (hourInRange(hour, shifts.guard.start, shifts.guard.end)) return "#ef4444"; // red
  if (hourInRange(hour, shifts.morning.start, shifts.morning.end)) return "#22c55e"; // green
  if (hourInRange(hour, shifts.afternoon.start, shifts.afternoon.end)) return "#22c55e"; // green
  return null;
}
}
...
export default function DashboardPage() {
  ...
  const kpis = useMemo(() => {
    ...
    const tppHealth = (() => {
      ...
      const capMax = Number(settings.tpp.capacityMax);
      const optMax = Number(settings.tpp.optimalMax);
      const limMax = Number(settings.tpp.limitMax);

      if (tpp6m < capMax) return { label: "Con Capacidad", color: "#22c55e" }; // green
      if (tpp6m >= capMax && tpp6m <= optMax) return { label: "Óptimo", color: "#2563eb" }; // blue
      if (tpp6m > optMax && tpp6m <= limMax) return { label: "Al Límite", color: "#f59e0b" }; // amber
      return { label: "Warning", color: "#ef4444" }; // red
    })();
            ...
            <CardContent className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={series.topAssignees} layout="vertical" margin={{ left: 60 }}>
                  ...
                  <Bar dataKey="tickets">
                    {series.topAssignees.map((e, i) => (
                      <Cell key={`cell-${i}`} fill={settings.team.includes(e.name) ? UI.primary : UI.warning} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
                ...
                <div className="grid grid-cols-6 gap-2">
                  {series.hourHeatMap.data.map((x) => (
                    <div
                      ...
                      style={{ ...heatBg(x.tickets, series.hourHeatMap.max), boxShadow: shiftBorderForHour(x.hour, settings.shifts) ? `inset 0 0 0 2px ${shiftBorderForHour(x.hour, settings.shifts)}` : undefined }}
                    >
                    ...
                    <tbody>
                      {series.weekHeatMap.matrix.map((row: any) => (
                        <tr key={row.hour}>
                          ...
                          {series.weekHeatMap.days.map((d) => {
                            const v = Number(row[d] || 0);
                            const border = shiftBorderForHour(Number(row.hour), settings.shifts);
                            const style = { ...heatBg(v, series.weekHeatMap.max), boxShadow: border ? `inset 0 0 0 2px ${border}` : undefined };
                            return (
